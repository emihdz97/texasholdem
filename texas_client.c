...
void who_win(int connection_fd);
...
int main(int argc, char * argv[])
{
    ...
    //recive and print who wins
    who_win(connection_fd);
...
//Handle the comunication for the gamble rounds
void handler(int connection_fd)
{
...
//Print the corresponding actions and wait for the client to select one
void options(float balance, float debt, int connection_fd)
{
    char action;
    float push_amount;
...
//Send to the server the corresponding action
void operation(char action, float push_amount, int connection_fd)
{
    char buffer[BUFFER_SIZE];
...
//Recives and print the balance of all players
void players_info(int connection_fd)
{
    float p1B, p2B, p3B;
    char buffer[BUFFER_SIZE];
...
//Recives and print three card for all players
void three_cards(int connection_fd)
{
    char card1[20];
    char card2[20];
...
//Recives and print one card for all players
void one_card(int connection_fd)
{
    char card1[20];
    char card2[20];
...
//Recives and print all the players card hands
void hand_reveals(int connection_fd)
{
    char card1[20];
    char card2[20];
...
//Recives and print the winner and the updated balance
void who_win(int connection_fd)
{
    char buffer[BUFFER_SIZE];
    int player_num;
    ...
    recv(connection_fd, buffer, BUFFER_SIZE, 0);
    sscanf(buffer, "%d %f", &player_num, &new_bal);
    printf("Player %d win\nYour new balance is: %f\n", player_num + 1, new_bal);
    sprintf(buffer, "OK");
    send(connection_fd, buffer, BUFFER_SIZE, 0);
}